...
const { hotspots, anomalies, forecasts, districtStats } = require('./dataLayer');
 ...
 */
function generateRecommendations(districtData, crimeCategory = null, riskLevel = null, trend = null) {
  const recs = []
  ...
  if (cyberZ > 2) {
    recs.push({
      ...
      reason: 'High density of cyber crime offences relative to statewide baseline statistics.',
    })
  }
  ...
  if (womenZ > 1.5 || (effectiveTrend === 'increasing' && womenTotal > womenMean)) {
    recs.push({
      ...
      reason: 'Elevated gender-related crimes and safety metrics indicating vulnerable pockets.',
    })
  }
  ...
  if (effectiveRisk === 'critical' || effectiveRisk === 'high') {
    recs.push({
      ...
      reason: 'Composite district hotspot index exceeds standard operational risk thresholds.',
    })
  }
  ...
  if (effectiveTrend === 'increasing' && fore) {
    recs.push({
      ...
      reason: 'Statistically significant positive YoY growth trend projected in the next period.',
    })
  }
  ...
  if (murderZ > 1.5) {
    recs.push({
      ...
      reason: 'Violent crime incident z-score exceeds safe operational limit guidelines.',
    })
  }
  ...
  if (districtData.scst > 100) {
    recs.push({
      ...
      reason: 'Elevated SC/ST atrocity reports relative to average statewide parameters.',
    })
  }
  ...
  if (districtData.pocso > 150) {
    recs.push({
      ...
      reason: 'High child protection and POCSO index counts require localized response cells.',
    })
  }
  ...
  if (recs.length === 0) {
    recs.push({
      ...
      reason: 'Routine monitoring parameters satisfied. Sustained operations recommended.',
    })
  }
 ...
 */
function generateExplainableAI(intent, sources, districts = [], crimeFields = []) {
  const methods = {
